...
def index():
    ...
    cur.execute('SELECT * FROM tech_stack_table')
    tech_stack_table = cur.fetchall()
    cur.close()
    return render_template('index.html', tech_stack_table=tech_stack_table)

@app.route('/add', methods=['POST'])
def add():
    new_component = request.form.get('new_component')
    cur = mysql.connection.cursor()
    cur.execute('INSERT INTO tech_stack_table (component) VALUES (%s)', [new_component])
